...
from 999music.utils.decorators.language import LanguageStart, languageCB
from 999music.utils.inline.help import (
    action_sub_menu,
    first_page,
    help_back_markup,
    private_help_panel,
...
from 999music.utils.inline.start import private_panel
...
async def helper_private(client: Client, update: Union[Message, types.CallbackQuery], _):
    ...
    if is_cb:
        await update.answer()
        # Smart wrapper uses Bot API (keeps colors) with Pyrogram fallback
...
async def back_to_main_cb(client: Client, CallbackQuery: types.CallbackQuery, _):
    ...
    user_mention = (
        ...
        f'{CallbackQuery.from_user.first_name}</a>'
    )
