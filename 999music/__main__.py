...
import config
from 999music import LOGGER, app, userbot
from 999music.core.call import VISHAL
from 999music.misc import sudo
from 999music.plugins import ALL_MODULES
from 999music.utils.database import get_banned_users, get_gbanned
from 999music.utils.cookie_handler import fetch_and_store_cookies
...
async def init():
    ...
    try:
        ...
        LOGGER("999music").info("ʏᴏᴜᴛᴜʙᴇ ᴄᴏᴏᴋɪᴇs ʟᴏᴀᴅᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ ✅")
    except Exception as e:
        LOGGER("999music").warning(f"⚠️ ᴄᴏᴏᴋɪᴇ ᴇʀʀᴏʀ: {e}")
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("999music.plugins" + all_module)

    LOGGER("999music.plugins").info("ᴍᴏᴅᴜʟᴇs ʟᴏᴀᴅᴇᴅ...")

    await userbot.start()
    await VISHAL.start()

    try:
        await VISHAL.stream_call("https://files.catbox.moe/rgbl8m.mp4")
    except NoActiveGroupCall:
        LOGGER("999music").error(
    ...
    LOGGER("999music").info("✅  Music Bot Started Successfully!")
    await idle()
    await app.stop()
    await userbot.stop()
    LOGGER("999music").info("sᴛᴏᴘᴘɪɴɢ ᴍᴜsɪᴄ ʙᴏᴛ ...")
            ...
            try:
                ...
                if _sync_db is not None:
                    try:
                        ...
                        LOGGER("999music").warning(f"DB stats error: {db_error}")
...
<div class="h"><h1>🎵 Vishal Music Bot</h1><div class="sb"><span class="dot"></span><span style="color:#00ff88;font-size:.9em">Online & Running</span></div></div>
<div class="sg">
...
<div class="f"><p>Auto-refreshes every 30s</p><p style="margin-top:8px"><a href="https://t.me/{config.BOT_USERNAME}">Open Bot</a> • <a href="https://t.me/Panda_king_HTbot">Developer</a></p></div>
</div></body></html>"""
            except Exception as e:
                LOGGER("999music").error(f"Flask home error: {e}")
        ...
        def api_stats():
            ...
            try:
                ...
                LOGGER("999music").error(f"Flask API stats error: {e}")
